...
const MODEL_REMAP = {
    ...
    ollama: {
        'claude-opus-4-6':    process.env.OLLAMA_OPUS_MODEL || 'deepseek-v4-pro',
        'claude-opus-4-7':    process.env.OLLAMA_OPUS_MODEL || 'deepseek-v4-pro',
        'claude-sonnet-4-6':  process.env.OLLAMA_SONNET_MODEL || 'deepseek-v4-flash',
        'claude-sonnet-4-5-20250929': process.env.OLLAMA_SONNET_MODEL || 'deepseek-v4-flash',
        'claude-haiku-4-5-20251001':  process.env.OLLAMA_HAIKU_MODEL || 'deepseek-v4-flash',
    },
};
...
export function startModelProxy({ targetUrl, apiKey, startPort = 3200, backends, defaultMode }) {
    return new Promise((resolve, reject) => {
        ...
        const initialNeedsTranslation = targetUrl.includes('ollama.com');
        ...
        if (backends) {
            for (const [name, cfg] of Object.entries(backends)) {
                allBackends[name] = {
                    ...
                    apiKey: cfg.apiKey,
                    useBearer: cfg.url.includes('openrouter') || cfg.url.includes('fireworks') || cfg.url.includes('ollama.com'),
                    needsTranslation: cfg.url.includes('ollama.com'),
                };
            }
        }
        ...
        function switchMode(name) {
            ...
            state.needsTranslation = b.needsTranslation || false;
        ...
        const server = createServer((clientReq, clientRes) => {
            ...
            clientReq.on('end', () => {
                ...
                // Ollama Cloud: translate Anthropic format → Ollama native format
                ...
                const proxyReq = httpsRequest(opts, (proxyRes) => {
                    ...
                    // Handle Ollama upstream errors — translate to Anthropic error format
                    if (isModelCall && state.needsTranslation && proxyRes.statusCode >= 400) {
                        const errChunks = [];
                        proxyRes.on('data', c => errChunks.push(c));
                        proxyRes.on('end', () => {
                            const errBody = Buffer.concat(errChunks).toString();
                            console.error(`[MODEL-PROXY] #${reqId} Ollama upstream error ${proxyRes.statusCode}: ${errBody.substring(0, 200)}`);
                            let errMsg = 'Ollama backend error';
                            try { errMsg = JSON.parse(errBody).error || errMsg; } catch {}
                            const anthropicErr = JSON.stringify({
                                type: 'error',
                                error: { type: 'api_error', message: errMsg },
                            });
                            clientRes.writeHead(proxyRes.statusCode, { 'content-type': 'application/json', 'content-length': Buffer.byteLength(anthropicErr) });
                            clientRes.end(anthropicErr);
                        });
                        return;
                    }

                    // Ollama streams NDJSON with content-type: application/json
                    // Detect streaming Ollama: needsTranslation + transfer-encoding chunked (no content-length)
                    const isOllamaStreaming = isModelCall && state.needsTranslation && !proxyRes.headers['content-length'];

                    if (isOllamaStreaming) {
                        // Ollama NDJSON → Anthropic SSE translation
                        const sseHeaders = { ...proxyRes.headers, 'content-type': 'text/event-stream' };
                        delete sseHeaders['content-length'];
                        delete sseHeaders['transfer-encoding'];
                        clientRes.writeHead(proxyRes.statusCode, sseHeaders);
                        const translator = new OllamaToAnthropicStream(
                            ollamaRequestModel || 'unknown',
                            (inp, out) => recordUsage(state.mode, inp, out)
                        );
                        proxyRes.pipe(translator).pipe(clientRes);
                        proxyRes.on('end', () => {
                            console.error(`[MODEL-PROXY] #${reqId} done in ${((Date.now() - t0) / 1000).toFixed(1)}s (ollama-stream, ${translator._inputTokens}in/${translator._outputTokens}out)`);
                        });
                    } else if (isModelCall && isSSE) {
                    ...
                    } else if (isModelCall && ct.includes('application/json') && state.needsTranslation) {
                        // Ollama JSON → Anthropic JSON translation (non-streaming)
                        const respChunks = [];
                        proxyRes.on('data', c => respChunks.push(c));
                        proxyRes.on('end', () => {
                            try {
                                const ollamaResp = JSON.parse(Buffer.concat(respChunks));
                                const anthropicResp = translateResponse(ollamaResp, ollamaRequestModel);
                                if (anthropicResp.usage) {
                                    recordUsage(state.mode, anthropicResp.usage.input_tokens, anthropicResp.usage.output_tokens);
                                }
                                const outBuf = Buffer.from(JSON.stringify(anthropicResp));
                                const outHeaders = { ...proxyRes.headers, 'content-type': 'application/json', 'content-length': outBuf.length };
                                clientRes.writeHead(proxyRes.statusCode, outHeaders);
                                clientRes.end(outBuf);
                                console.error(`[MODEL-PROXY] #${reqId} done in ${((Date.now() - t0) / 1000).toFixed(1)}s (ollama-json, ${outBuf.length}b)`);
                            } catch (e) {
                                console.error(`[MODEL-PROXY] #${reqId} Ollama response translate error: ${e.message}`);
