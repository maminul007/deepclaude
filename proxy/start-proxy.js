...
const BACKEND_DEFS = {
    ...
    ollama: { url: process.env.OLLAMA_HOST || 'https://ollama.com', keyEnv: 'OLLAMA_API_KEY' },
};
...
if (targetUrl && apiKey) {
    // Legacy single-backend mode (with optional --mode flag)
    const args = process.argv.slice(4);
    const modeIdx = args.indexOf('--mode');
    const legacyMode = modeIdx >= 0 ? args[modeIdx + 1] : undefined;
    ...
    const { port } = await startModelProxy({
        ...
        defaultMode: legacyMode || (hasBackends ? undefined : undefined),
    });
    console.log(port);
} else {
